...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_packed = nn.utils.rnn.pack_padded_sequence(X, torch.tensor(source_lengths))   # 使网络专注于实际的数据
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens, total_length=source_padded.size(0))  # 还原为原始的填充张量
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # 交换维度
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        combined_outputs = torch.stack(combined_outputs)    # 将列表的tensor转化为tensor
        ### END YOUR CODE
